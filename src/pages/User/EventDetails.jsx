...
export default function EventDetails() {
    ...
    const isFull = event?.capacity <= event?.registeredCount;
    ...
    if (isFree) {
        ...
        alert("Redirecting to payment gateway... (in progress)");
    }
    };
  ...
  const handleConfirmRegister = async () => {
    try {
        ...
        `${BaseUrl}/events/register/${event.id}`,
        {},
    ...
    useEffect(() => {
        const checkRegistration = async () => {
            ...
            const res = await axios.get(`${BaseUrl}/events/registrations/check`, {
                params: {
                eventId: event?.id,
                },
                headers: {
    ...
    <div className="min-h-screen bg-[#f8fafc] pb-12">
      ...
      <div className="sticky top-0 z-20 bg-white/80 backdrop-blur-md border-b border-[#e2e8f0]
        ...
        <button
            ...
            className={`ml-auto flex items-center gap-1.5 text-md font-semibold px-4 py-2.5 rounded-xl transition
                ${
                loading
                    ...
                    : "bg-primary text-slate-200 hover:bg-indigo-700"
                }`}
            >
            <ShieldIcon />
            {loading ? "Checking..." : isRegistered ? "Registered" : isFull ? "Full" : "Register"}
        </button>
      </div>
